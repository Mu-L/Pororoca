...
public class SortedObservableList<T> : IList, IList<T>, INotifyCollectionChanged, INotifyPropertyChanged, IReadOnlyList<T>
{
    ...
    public void AddAll(IEnumerable<T> elements, bool isSorted = false)
    {
        ...
        {
            if (!this.SequenceEqual(this.OrderBy(x => x, comparer)))
                throw new InvalidDataException("UNSORTED");
        ...
        while (seIndex < elementCount && insertStartIndex < count)
        {
            ...
            else
            {
                ...
                {
                    var insertionElements = sortedElements.ToArray()[seStartIndex..(seStartIndex + insertionCount)];
    ...
    public int BinarySearch(T element, Comparison<T> comparison) =>
        BinarySearch(list, 0, list.Count, element, comparison);
    ...
    {
        int v = this.list.BinarySearch(element, this.comparer);
        return v >= 0 ? v : -1;
    }
            ...
            if (comparisonResult == 0)
            {
                if (removingStartIndex > listIndex + 1)
                {
                    ...
                    var removedElements = collectionChangedHandlers != null
                        ? list[removingStartIndex..(removingStartIndex + count)].ToArray()
                        : null;
    ...
    public IList<T> Reverse() => ((IList<T>)this).Reverse().ToList();
    ...
    public IList<T> ReverseAsReadOnly() => ((IList<T>)this).Reverse().ToList().AsReadOnly();
    ...
    IList<T> Sort(IEnumerable<T> elements, bool isSorted)
    {
        if (elements is SortedObservableList<T> sortedList)
        {
            return sortedList;
        }
        if (!isSorted)
